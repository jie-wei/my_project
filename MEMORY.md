...
[LEARN:workflow] Spec-then-plan protocol: ask user (3-5 questions) -> create `quality_reports/specs/YYYY-MM-DD_description.md` with MUST/SHOULD/MAY requirements -> declare clarity status (CLEAR/ASSUMED/BLOCKED) -> get approval -> then draft plan.
...
[LEARN:documentation] Documentation must be generic (framework-oriented) not prescriptive. Provide templates with examples for multiple workflows (LaTeX, Python, Jupyter), let users customize. No "thou shalt" rules.
...
[LEARN:design] Generic means working for any academic workflow: pure LaTeX (no Quarto), pure Python (no R), any domain (not just one field). Test recommendations across use cases.
...
[LEARN:meta] Dogfooding principles must be enforced: plan-first, spec-then-plan, quality gates, session logs -- we follow our own workflow.
