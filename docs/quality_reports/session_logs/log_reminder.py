...
def load_state(state_path: Path) -> dict:
    ...
    try:
        ...
        return {"counter": 0, "last_mtime": 0.0, "reminded": False, "no_log_reminded": False, "no_log_date": ""}
...
def find_latest_log(project_dir: str) -> tuple[Path | None, float]:
    ...
    log_dir = Path(project_dir) / "docs" / "quality_reports" / "session_logs"
...
def main():
    ...
    # Case 1: No session log exists at all — remind once per day
    if latest_log is None:
        if state.get("no_log_date") != today:
            state["no_log_reminded"] = True
            state["no_log_date"] = today
    ...
    if current_mtime != state["last_mtime"]:
        state = {"counter": 0, "last_mtime": current_mtime, "reminded": False, "no_log_reminded": False, "no_log_date": ""}
        save_state(state_path, state)
        sys.exit(0)
