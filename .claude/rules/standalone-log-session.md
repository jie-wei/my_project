...
Each Claude Code session gets its own log file. The `reminder-log` hook creates a stub automatically on the first Stop event. Rename the stub to add a short description (keep the existing prefix). The `{session_hash}` is derived from the session's unique ID, so any hook can independently find the correct log.
