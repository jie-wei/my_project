...
- You actively develop research papers, analysis code, and documentation
- You accumulate learnings specific to your setup and workflow
- You test new features and iterate on infrastructure
- You have institutional context (your department, field, specific tools)

### Identity 2: Public Template
- Others fork this repo to bootstrap their own academic workflows
- They use different domains (biology, physics, CS, economics, humanities)
- They use different tools (pure LaTeX, Python, R, Jupyter, mixed)
- They need generic patterns, not your specific decisions
...
- Documentation standards (update README+CLAUDE.md together)
- Rules that adapt to user context (path-scoped rules)

**SPECIFIC (keep local or gitignore):**
- Machine-specific paths (TEXINPUTS for custom preamble locations)
- Tool version pinning (specific package versions that vary by setup)
- Institutional requirements (thesis format, departmental standards)
- Personal preferences (stricter quality thresholds for this project)
...
- Documentation patterns: `[LEARN:documentation] Update README+CLAUDE.md together`
...
**What goes here:**
- Machine setup: `[LEARN:latex] XeLaTeX on this machine requires specific TEXINPUTS path`
- Tool quirks: `[LEARN:tools] Package X version Y has a bug, use version Z`
- Local paths: `[LEARN:files] Bibliography at ~/path/to/references.bib`
- Personal workflow: `[LEARN:workflow] I prefer 90/100 for manuscripts, 80/100 for explorations`
...
- Do: Update README and CLAUDE.md together when adding features
...
Always use XeLaTeX with TEXINPUTS=../specific/path for department slides.
```

**Good (framework-oriented):**
```markdown
# LaTeX Compilation Rule
Use project-specific TEXINPUTS if preambles are in a separate directory.
...
**Bad (single use case):**
```markdown
Example: Panel data regression with fixed effects
```

**Good (diverse use cases):**
```markdown
Examples:
- Economics: Panel regression with fixed effects
- Biology: Lab protocol validation pipeline
- Physics: Numerical simulation workflow
- CS: Algorithm benchmarking framework
```
...
**Bad (prescriptive):**
```markdown
Your bibliography MUST be named references.bib and live in paper/.
```
...
> "This workflow was developed for [a specific course/project]..."

As long as it's clear this is ONE example, not THE requirement.

### CLAUDE.md Can Have Placeholders

The template CLAUDE.md has customizable sections -- this is correct. Users fill them in.

### Documentation Can Reference Original Use Case

Pedagogically valuable to show a real-world example:
> "Case Study: [description of a multi-file project with LaTeX + Python + data pipeline]"
...
| Templates (generic) | Yes | docs/templates/ | Yes |
| Skills (generic) | Yes | .claude/skills/ | Yes |
| Rules (path-scoped, generic) | Yes | .claude/rules/ | Yes |
| Agents (specialized reviewers) | Yes | .claude/agents/ | Yes |
| Hooks (generic behavior) | Yes | .claude/hooks/ | Yes |
| Session logs | Yes | docs/quality_reports/session_logs/ | Yes |
| Plans | Yes | docs/quality_reports/plans/ | Yes |
...
1. Your working project (specific, contextual, evolving)
...
- Dogfood your own workflow (plan-first, spec-then-plan, quality gates)
- Document with examples from multiple domains (not just your use case)
- Review periodically: promote generic patterns, refine specific ones

**When in doubt:** Ask "Would a researcher in a completely different field forking this repo benefit from this knowledge?" If yes -- MEMORY.md. If no -- personal-memory.md.
